...
{
    //Initialisation
    private enum View { List, Beat } // Creates an object with list and beat in it.

    private Vector2 _scroll = new Vector2();
    private int _currentIndex = -1;
    private View _view;

    [MenuItem("SFAS/Show Story Editor")] //Heading
    public static void ShowStoryEditor()
    {
        GetWindow(typeof(StoryEditor));
    }

    void OnGUI()//Called for handling UI events
    {
    ...
    private void OnGUI_ListView(SerializedProperty beatList)
    {
        ...
        {
            AddBeat(beatList, 1, "First Story Beat");//Adds the first beat of the story
        }
    ...
    private int FindIndexOfBeatId(SerializedProperty beatList, int beatId)//finds the array element of the id.
    {
        int result = -1;
